...
(function (marmoset) {
    ...
    function Archive(a) {
        ...
        for (a = new ByteStream(a) ; !a.empty() ;) {
            var block = {};
            block.name = a.readCString();
            block.type = a.readCString();
            var needDecompress = a.readUint32(),
                length = a.readUint32(),
                decompressedLength = a.readUint32();
            block.data = a.readBytes(length);
        ...
        //var sceneJason = this.files["scene.json"].data;
           
        //alert((new ByteStream(sceneJason)).asString());
    }
    ...
    function Framebuffer(glContext, desc) {
        ...
        this.valid = desc && desc.ignoreStatus || glContext.checkFramebufferStatus(glContext.FRAMEBUFFER) == glContext.FRAMEBUFFER_COMPLETE;
    ...
    function Lights(lights, view) {
        ...
        for (var i in lights) this[i] = lights[i];
    ...
    function Material(glContext, achieve, material) {
        ...
        this.shader = glContext.shaderCache.fromURLs("matvert.glsl", "matfrag.glsl", prefix);
    ...
    MeshRenderable.prototype.draw = function (scene) {
        ...
        if (this.material.bind(scene)) {
            ...
            var bVColor = this.mesh.vertexColor && undefined !== a.vColor;
            if (bVColor) {
    ...
    PostRender.prototype.fillScreen = function (a) {
        ...
        b.vertexAttribPointer(a, 2, b.FLOAT, false, 0, 0);
    ...
    Scene.prototype.load = function (archive) {
        ...
        if (undefined !== jsonData) {
            if (!archive.checkSignature(jsonData)) {
                return false;
            }
    ...
    ShaderCache.prototype.fromURLs = function (vertCodeName, fragCodeName, prefix) {
        var code = "";
        if (prefix !== undefined) {
                ...
                this.fetch(includes[i].path, function (result) {
                    ...
                    if (needToFetch <= 0) {
                        for (var i = includes.length - 1; i>=0; --i) {
                            code = code.substring(0, includes[i].offset) + includes[i].src + code.substring(includes[i].offset);
                        }
                        callback(code)
                    }
                }.bind(includes[i]));
    ...
    ShadowCollector.prototype.bindDepthTexture = function (depthTex, count) {
        if (this.shadowCount > count) {
            this.depthTextures[count].bind(depthTex);
        }
    };
    ...
    function Sky(glContext, archieve, sky) {
        ...
        if (3 == this.backgroundMode) {
            ...
            if(glContext.ext.textureHalf && glContext.ext.textureHalfLinear){
                this.backgroundTexture.loadArray(null, glContexta.RGB, glContext.ext.textureHalf.HALF_FLOAT_OES);
            ...
            shader.build("precision highp float; varying vec2 tc; attribute vec4 p; void main(){ gl_Position=p; tc=vec2(0.5,0.5/8.0)*p.xy+vec2(0.5,6.5/8.0); }",
            ...
            shader.bind();
            glContexta.uniform1f(shader.params.b, 7 * Math.sqrt(this.backgroundBrightness));
            ...
            glContext.enableVertexAttribArray(shader.attribs.p);
            glContext.vertexAttribPointer(shader.attribs.p, 4, glContext.FLOAT, false, 0, 0);
            glContext.drawArrays(glContext.TRIANGLES, 0, 3);
            glContext.disableVertexAttribArray(shader.attribs.p)
        }
    }
    ...
    Texture.prototype.loadArray = function (image, type, innerType) {
        var glContext = this.gl;
        this.id = glContext.createTexture();
        glContext.bindTexture(this.type, this.id);
        glContext.pixelStorei(glContext.UNPACK_FLIP_Y_WEBGL, true);
        glContext.texImage2D(this.type, 0, type || glContext.RGBA, this.desc.width, this.desc.height, 0, type || glContext.RGBA, innerType || glContext.UNSIGNED_BYTE, image || null);
        this.setParams();
        glContext.bindTexture(this.type, null)
    };
